...
conexion = psycopg2.connect(database='scr_bdd_txt', user='miguel', password='12345', host='localhost', port='5432')
cur = conexion.cursor()


for vuelo in vuelos:
    hora_vuelo = vuelo.find_element_by_xpath('.//div[@class="departure"]/time').get_attribute('datetime')
    valor_vuelo = vuelo.find_element_by_xpath('.//span[@class="value"]').text
    cur.execute("INSERT INTO vuelos (horas, precio) VALUES ('"+ str(hora_vuelo) + "', '" + str(valor_vuelo) +"')")

cur.execute("SELECT * FROM vuelos")
tablas = cur.fetchall()

for tabla in tablas:
    elemento = "Hora de vuelo: " + tabla[0] + "\nPrecio: " + tabla[1] + "\n"
    print(elemento)
    archivo = open('my_first_txt.txt', 'a+')
    with archivo:
        writer = archivo.write(elemento)
